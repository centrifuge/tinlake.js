...
describe('solver tests', async () => {
  describe('solver', async () => {
    it('should return an optimal solution when limited by the max TIN ratio', async () => {
      const state = {
        netAssetValue: new BN(800).mul(new BN(10).pow(new BN(12))),
        reserve: new BN(200).mul(new BN(10).pow(new BN(12))),
        seniorAsset: new BN(800).mul(new BN(10).pow(new BN(12))),
        minTinRatio: new BN(0.15 * 10**7),
        maxTinRatio: new BN(0.2 * 10**7),
        maxReserve: new BN(10000).mul(new BN(10).pow(new BN(12))),
      }

      const orderState = {
        tinRedeemOrder: new BN(100).mul(new BN(10).pow(new BN(12))),
        dropRedeemOrder: new BN(300).mul(new BN(10).pow(new BN(12))),
        tinInvestOrder: new BN(200).mul(new BN(10).pow(new BN(12))),
        dropInvestOrder: new BN(400).mul(new BN(10).pow(new BN(12))),
      }
